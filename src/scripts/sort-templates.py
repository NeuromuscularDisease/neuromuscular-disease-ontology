    ...
    with open(path, "r") as tsv:
        ...
        for row in reader:
            ...
            if id_idx is not None:
                term_id = row[id_idx]
                if term_id in ids:
                    # For now, just give warning for duplicates instead of raise Exception
                    #raise Exception(
...
def main():
    parser = argparse.ArgumentParser(description="Sort TSV file by ID field")
    parser.add_argument("input", type=str, help="TSV file to sort")
    args = parser.parse_args()

    sort_template(args.input)
